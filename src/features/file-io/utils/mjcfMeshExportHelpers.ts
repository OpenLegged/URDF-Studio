...
export function isMjcfNativeMeshPath(meshPath: string): boolean {
  return /\.(?:obj|stl|msh)$/i.test(meshPath);
}

export function collectMeshPathAliases(meshPath: string): string[] {
...
export function buildConvertedMeshBasePath(normalizedPath: string): string {
...
export function buildConvertedMeshExportPath(
  meshPath: string,
...
export interface ExtractedVisualMeshVariant extends MjcfVisualMeshVariant {
  blob: Blob;
}

export interface PendingVisualMeshVariant {
  geometry: THREE.BufferGeometry;
  material: THREE.Material;
  meshName: string;
}

export const COPLANAR_ANCHOR_BAKE_OFFSET = 1e-4;

export type GeometryGroup = THREE.BufferGeometry['groups'][number];

export type GeometryAttribute = THREE.BufferAttribute | THREE.InterleavedBufferAttribute;

export function sanitizeVariantSegment(value: string | undefined): string {
  return String(value || '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '_')
    .replace(/^_+|_+$/g, '');
...
export interface ConvertedVisualVariantPathOptions {
  meshPath: string;
...
export function buildConvertedVisualVariantPath({
  meshPath,
  materialName,
  variantIndex,
  usedPaths,
...
export function colorToHex(color: THREE.Color | undefined): string | undefined {
  if (!color) {
    return undefined;
  }
...
export function exportObjBlob(objExporter: OBJExporter, object: THREE.Object3D): Blob | null {
...
export type MeshFormatKind = 'auto' | 'obj' | 'stl';

export type MeshExtension = '.obj' | '.stl';

export interface MeshExporters {
  obj: OBJExporter;
  stl: STLExporter;
}

export function exportStlBlob(stlExporter: STLExporter, object: THREE.Object3D): Blob | null {
...
export function exportMeshBlob(
...
export function isColorLike(color: unknown): color is THREE.Color {
...
export function getMaterialColor(material: THREE.Material): THREE.Color | undefined {
  return 'color' in material && isColorLike(material.color) ? material.color : undefined;
}

export function isBufferGeometryLike(geometry: unknown): geometry is THREE.BufferGeometry {
...
export function getAttributeArray(attribute: GeometryAttribute): ArrayLike<number> {
  return attribute instanceof THREE.InterleavedBufferAttribute
    ? attribute.data.array
    : attribute.array;
}

export function cloneAttributeSubset(
...
export function getUsableGeometryIndex(geometry: THREE.BufferGeometry): GeometryAttribute | null {
  const index = geometry.getIndex();
  return index && index.count > 0 ? index : null;
}

export function getTriangleVertexIndex(
...
export function buildTriangleVertexKey(
...
export function buildTriangleKey(
  positionArray: ArrayLike<number>,
  itemSize: number,
  indexArray: ArrayLike<number> | null,
  start: number,
...
export function pickPreferredTriangleOwner(
...
export function buildDuplicateTriangleOwnerMap(
...
export function buildIndexedGeometrySubset(
  sourceGeometry: THREE.BufferGeometry,
...
export function buildNonIndexedGeometrySubset(
  sourceGeometry: THREE.BufferGeometry,
  relevantGroups: readonly GeometryGroup[],
  preferredTriangleOwners: ReadonlyMap<string, number>,
  targetMaterialIndex: number,
...
export function extractGeometryForMaterial(
  sourceGeometry: THREE.BufferGeometry,
  materials: readonly THREE.Material[],
...
export function buildSceneVariantTriangleOwnerMap(
...
export function buildSceneVariantGeometrySubset(
  sourceGeometry: THREE.BufferGeometry,
  preferredTriangleOwners: ReadonlyMap<string, number>,
...
export function shouldBakeCoplanarAnchorOffset(
  materials: readonly THREE.Material[],
...
export function applyGeometryNormalOffset(geometry: THREE.BufferGeometry, distance: number): void {
...
export function createBakedVariantMesh(
  mesh: THREE.Mesh,
  material: THREE.Material,
  materialIndex?: number,
): THREE.Mesh | null {
...
export interface ExtractVisualMeshVariantsOptions {
...
export function extractVisualMeshVariants({
  meshObject,
  sourceMeshPath,
  usedArchivePaths,
  exporters,
...
export function registerInlineMeshBlobUrls(
...
export function collectReferencedMeshUsage(robot: RobotState): Map<string, ReferencedMeshUsage> {
...
export function findVisualGeometryByMeshPath(
  robot: RobotState,
  meshPath: string,
...
export function getInlineMeshBlob(
...
export function shouldParseColladaInProcessForMjcfExport(meshPath: string): boolean {
  return /\.dae$/i.test(meshPath) && typeof Worker === 'undefined' && typeof window === 'undefined';
}

export async function loadColladaMeshInProcessForMjcfExport(
  meshPath: string,
...
export async function hashMeshBytes(bytes: Uint8Array): Promise<string> {
...
export function markSharedMeshReuse(
  sourceMeshPath: string,
...
export function registerNativeMeshPassThroughOverride(
  sourceMeshPath: string,
...
export function createMjcfMeshExportError(meshPath: string, cause: unknown): Error {
...
export async function prepareSharedNativeMeshReuse(
