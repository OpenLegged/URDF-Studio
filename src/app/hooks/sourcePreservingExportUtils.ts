...
import { assertPatchableXacroModel } from './source-preserving-export/xacroPatchPolicy';
...
export interface ResolveSourcePreservingExportContentOptions {
  ...
  /** Restore caller-owned markers before the resolver validates and returns content. */
  finalizePatchedContent?: (content: string) => string;
  xacroSourceModelMode?: 'strict' | 'direct-changes';
}
...
interface PatchUrdfLikeSourceOptions {
...
interface PatchSourceContentOptions {
  format: SourcePreservingExportFormat;
  sourceContent: string;
  generatedContent: string;
  sourceRobot: RobotState | null;
...
function stripSourceEncodingMetadata<T>(value: T): T {
  if (Array.isArray(value)) {
    return value.map((item) => stripSourceEncodingMetadata(item)) as T;
  }
  ...
  return Object.fromEntries(
    Object.entries(value as Record<string, unknown>)
      ...
      .map(([key, nestedValue]) => [key, stripSourceEncodingMetadata(nestedValue)]),
  ) as T;
}

function normalizeForSnapshot(robot: RobotState): RobotState {
  return {
    ...stripSourceEncodingMetadata(robot),
...
function stableNormalize(value: unknown): unknown {
  ...
  const skippedKeys = new Set(['visible', 'selection', 'quaternion', 'materialSource']);
  return Object.fromEntries(
    Object.entries(value as Record<string, unknown>)
...
function patchUrdfLikeSource({
  ...
  sourceRobot,
  generatedRobot,
  xacroSourceModelMode = 'strict',
}: PatchUrdfLikeSourceOptions): string {
  const sourceRoot = findRootElement(sourceContent, 'robot');
  const generatedRoot = findRootElement(generatedContent, 'robot');
  if (!sourceRoot || !generatedRoot) {
  ...
  if (format === 'xacro') {
    assertPatchableXacroModel({
      sourceContent: patched,
      sourceChildKeys: new Set(sourceChildrenByKey.keys()),
      generatedChildren,
      changedKeys,
      mode: xacroSourceModelMode,
    });
  }
...
function patchSourceContent({
  ...
  generatedRobot,
  xacroSourceModelMode = 'strict',
}: PatchSourceContentOptions): string {
  switch (format) {
    case 'urdf':
...
function resolveMatchingSourceContent(
  options: ResolveSourcePreservingExportContentOptions,
  sourceContent: string,
): SourcePreservingExportResult {
  const { format, sourceFile, generatedContent } = options;
  const currentSourceContent = sourceFile?.content ?? sourceContent;
  const baseContent = format === 'xacro'
    ? patchMatchingXacroSource(currentSourceContent, generatedContent)
    : currentSourceContent;
  const content = options.finalizePatchedContent?.(baseContent) ?? baseContent;
  if (format === 'xacro' || options.finalizePatchedContent) {
    validatePatchedContent(options, content);
  }
  return {
    content,
...
export function resolveSourcePreservingExportContent(
  ...
  if (sourceRobot && buildRobotSnapshot(sourceRobot) === buildRobotSnapshot(generatedRobot)) {
    return resolveMatchingSourceContent(options, sourceContent);
  }
  ...
  try {
    patchedContent = patchSourceContent({
      format,
      sourceContent: sourceFile.content ?? sourceContent,
      generatedContent,
      sourceRobot,
      generatedRobot,
      xacroSourceModelMode: options.xacroSourceModelMode ?? 'strict',
    });
  } catch (error) {
  ...
  patchedContent = options.finalizePatchedContent?.(patchedContent) ?? patchedContent;
  validatePatchedContent(options, patchedContent);
