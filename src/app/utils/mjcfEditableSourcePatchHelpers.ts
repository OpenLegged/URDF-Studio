...
export function getPreferredNewline(sourceContent: string): string {
  return sourceContent.includes('\r\n') ? '\r\n' : '\n';
}

export function getLineStart(sourceContent: string, index: number): number {
  let cursor = index;
  while (cursor > 0) {
...
export function getIndentAt(sourceContent: string, index: number): string {
