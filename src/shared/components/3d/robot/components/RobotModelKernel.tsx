...
import { LinkIkTransformControls } from '@/shared/components/3d/LinkIkTransformControls';
import { SceneCompileWarmup } from '@/shared/components/3d/scene/SceneCompileWarmup';
...
import { GeometryTransformControls } from '@/shared/components/3d/robot/components/CollisionTransformControls';
...
import { useRendererBackend } from '@/shared/components/3d/robot/hooks/useRendererBackend';
import { useRobotHoverLock } from '@/shared/components/3d/robot/hooks/useRobotHoverLock';
import { useHighlightManager } from '@/shared/components/3d/robot/hooks/useHighlightManager';
import { useCameraFocus } from '@/shared/components/3d/robot/hooks/useCameraFocus';
import { useMouseInteraction } from '@/shared/components/3d/robot/hooks/useMouseInteraction';
import { useHoverDetection } from '@/shared/components/3d/robot/hooks/useHoverDetection';
import { useVisualizationEffects } from '@/shared/components/3d/robot/hooks/useVisualizationEffects';
import { useRobotModelIkState } from '@/shared/components/3d/robot/hooks/useRobotModelIkState';
import { useRobotModelPaintInteraction } from '@/shared/components/3d/robot/hooks/useRobotModelPaintInteraction';
import { useExternalHoverHighlightSync } from '@/shared/components/3d/robot/hooks/useExternalHoverHighlightSync';
import { useRobotModelRuntimeRegistration } from '@/shared/components/3d/robot/hooks/useRobotModelRuntimeRegistration';
import { resolveCameraAutoFrameLoadScopeKey } from '@/shared/components/3d/robot/utils/cameraAutoFrame';
import {
  createRuntimeSceneLinkMetadataState,
  resolveRuntimeSceneLinkMetadataState,
} from '@/shared/components/3d/robot/utils/runtimeSceneMetadata';
import { resolveViewerRobotSourceFormat } from '@/shared/components/3d/robot/renderers/sourceFormat';
import { shouldEnableViewerSceneCompileWarmup } from '@/shared/components/3d/robot/utils/sceneCompileWarmupPolicy';
import { canTransformGeometry } from '@/shared/components/3d/robot/utils/geometryTransformPolicy';
...
export const RobotModelKernel: React.FC<RobotModelKernelProps> = memo(
  ({
    ...
    jointControlsEnabled = true,
    rootTransformActive = false,
    hasRootTransformControls = false,
    rootTransform,
    contentTransform,
    ...
    const { frozen: interactionHoverFrozen, setFrozen: setOwnedHoverFrozen } = useRobotHoverLock({
      enabled: interactionEnabled,
      externalFrozen: hoverFrozen,
      onFrozenChange: onHoverFrozenChange,
    });
    ...
    // ============================================================
    useHoverDetection({
      hoverFrozen: interactionHoverFrozen,
      robot,
      robotVersion,
      toolMode,
    ...
    const assemblyTransform = cloneAssemblyTransform(rootTransform);
    ...
    return (
      <>
        ...
        {renderSceneAttachment?.({ robot, assemblyRoot, directComponentRoot, requestSceneRefresh })}
        {!snapshotRenderActive &&
          !hasRootTransformControls &&
          !transformLocked &&
          transformMode !== 'select' &&
