...
function createLinkedSnapshotCaptureSignal(
  requestSignal: AbortSignal | undefined,
  lifecycleSignal: AbortSignal,
) {
  const controller = new AbortController();
  const handleAbort = () => controller.abort();
  const signals = requestSignal ? [requestSignal, lifecycleSignal] : [lifecycleSignal];
...
export interface SnapshotManagerRuntimeProps {
...
export const SnapshotManagerRuntime = ({
  actionRef,
  ...
  useEffect(() => {
    ...
    const captureAction: SnapshotCaptureAction = async (requestedOptions) => {
      const linkedSignal = createLinkedSnapshotCaptureSignal(
        requestedOptions?.signal,
        lifecycleAbortController.signal,
      );
      const runtimeRequest: SnapshotCaptureRequest = {
        ...(requestedOptions ?? {}),
        signal: linkedSignal.signal,
      };
      try {
        const capture = await runSnapshotCapture(runtimeRequest, normalizeSnapshotCaptureOptions);
        await downloadCanvas(capture.canvas, capture.options, {
          signal: runtimeRequest.signal,
          onProgress: runtimeRequest.onProgress,
        });
      } finally {
